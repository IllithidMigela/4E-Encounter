...
interface Props {
  ...
  /** 遭遇模拟器注入：速览威能 chip 的 ⚔ 攻击入口；缺省（车卡器纯速览）不显示攻击按钮 */
  onUsePower?: (p: Entry) => void;
}

const POWER_CATS: (keyof PowerSlots)[] = ["atWill", "encounter", "daily", "utility", "special"];

export default function OverviewView({ layout, char, setChar, onUsePower }: Props) {
    ...
    <div className={"glance" + (layout === "double" ? " double" : "")}>
      <div className="gl-shell">
        ...
        <div className="glance-main">
          ...
          <GlancePowers char={char} setChar={setChar} d={d} onUsePower={onUsePower} />
        ...
        <nav className="glance-nav" aria-label="分区导航">
          <button
            type="button"
            className="glance-nav-btn glance-nav-top"
            title="回到顶部"
            onClick={() => document.querySelector(".glance")?.scrollIntoView({ behavior: "smooth", block: "start" })}
          >
            <span className="material-symbols-outlined">vertical_align_top</span>
          </button>
          {[
            { label: "生命", target: "z-vit" },
            { label: "防御", target: "z-def" },
            { label: "攻击", target: "z-atk" },
            { label: "属性", target: "z-abi" },
            { label: "威能", target: "z-pow" },
            { label: "专长", target: "z-feat" },
            { label: "物品", target: "z-item" },
          ].map((item) => (
            <button
              key={item.target}
              type="button"
              className="glance-nav-btn"
              title={item.label}
